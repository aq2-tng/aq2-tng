...
// JBravo.
  ...
  if (!ent->client->chase_mode)
	...
	{
	  ent->client->ps.stats[STAT_GRENADE_ICON] =
	    gi.imageindex ("a_m61frag");
      ...
      if (ent->client->pers.weapon)
	{
	  switch (ent->client->curr_weap)
	    ...
	    case KNIFE_NUM:
	      {
		ent->client->ps.stats[STAT_AMMO_ICON] =
		  gi.imageindex ("w_knife");
		ent->client->ps.stats[STAT_AMMO] =
		  ent->client->pers.
		  inventory[ITEM_INDEX (FindItem (KNIFE_NAME))];
		break;
	      }
	    case GRENADE_NUM:
	      {
		...
		ent->client->ps.stats[STAT_AMMO] =
		  ent->client->pers.
		  inventory[ITEM_INDEX (FindItem (GRENADE_NAME))];
		break;
	      }
