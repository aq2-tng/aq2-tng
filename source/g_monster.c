...
// g_monster.c
//
// $Id: g_monster.c,v 1.3 2001/09/28 13:48:34 ra Exp $
//
//-----------------------------------------------------------------------------
// $Log: g_monster.c,v $
...
void
monster_fire_grenade (edict_t * self, vec3_t start, vec3_t aimdir, int damage,
...
void
monster_fire_rocket (edict_t * self, vec3_t start, vec3_t dir, int damage,
		     int speed, int flashtype)
{
...
void
monster_fire_railgun (edict_t * self, vec3_t start, vec3_t aimdir, int damage,
...
void
monster_triggered_spawn_use (edict_t * self, edict_t * other,
			     edict_t * activator)
{
  ...
  if (self->target)
    {
      ...
      notcombat = false;
      fixup = false;
      while ((target = G_Find (target, FOFS (targetname), self->target)) !=
	     NULL)
	{
