...
void
G_ProjectSource (vec3_t point, vec3_t distance, vec3_t forward, vec3_t right,
		 vec3_t result)
{
...
G_TouchTriggers (edict_t * ent)
{
  ...
  num =
    gi.BoxEdicts (ent->absmin, ent->absmax, touch, MAX_EDICTS, AREA_TRIGGERS);
...
KillBox (edict_t * ent)
{
  ...
  while (1)
    {
      tr =
	gi.trace (ent->s.origin, ent->mins, ent->maxs, ent->s.origin, NULL,
		  MASK_PLAYERSOLID);
      if (!tr.ent)
	break;
