...
//
// $Id: g_xmisc.c,v 1.4 2001/09/28 13:48:34 ra Exp $
//
//-----------------------------------------------------------------------------
// $Log: g_xmisc.c,v $
...
// AQ:TNG - JBravo adding FF after rounds
		       ...
		       (team_round_going && ff_afterround->value)))
		    {
		      gi.sound (ent, CHAN_WEAPON,
				gi.soundindex ("weapons/swish.wav"), 1,
				ATTN_NORM, 0);
		      return;
		    }
		}
  ...
  gi.sound (ent, CHAN_WEAPON, gi.soundindex ("weapons/swish.wav"), 1,
	    ATTN_NORM, 0);
...
void
PlayRandomInsaneSound (edict_t * ent)
{
  int n;
  char buffer[1000];
...
edict_t *
findblock (edict_t * from, vec3_t p1, vec3_t p2)
{
  vec3_t eorg;
  int j;
...
PrecacheUserSounds (void)
{
  ...
  if (soundlist == NULL)
  ...
  else
    {
      ...
      for (i = 0;
	   (fgets (buf, MAX_STR_LEN - 10, soundlist) != NULL) && (i < 100);)
	{
  ...
  for (i = 1; i <= game.maxclients; i++)
    {
	  ...
	  && (Q_stricmp (other->client->pers.netname, name) == 0))
	{
	  found = other;
	  break;
	}
...
int
GetClientInternalNumber (edict_t * ent)
{
  int i, j;
...
*/
void
KickClient (edict_t * target, char *reason)
{
  int j;
  char buffer[512];
...
//
qboolean
ParseStartFile (char *filename, parse_t * parse)
{
...
char *
ParseNextToken (parse_t * parse, char *seperator)
{
  char *dummy;
...
qboolean
_seekinisection (FILE * afile, char *section)
{
  inistr buf, comp;
...
char *
_readinivalue (FILE * afile, char *section, char *key, char *value)
{
  inistr buf, akey;
  char *p;
...
int
ReadIniSection (ini_t * ini, char *section, char buf[][INI_STR_LEN],
		int maxarraylen)
{
  int i;
  char *p;
...
char *
ReadIniStr (ini_t * ini, char *section, char *key, char *value,
	    char *defvalue)
{
...
int
ReadIniInt (ini_t * ini, char *section, char *key, int defvalue)
{
  inistr value;
