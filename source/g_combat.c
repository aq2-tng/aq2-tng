...
CanDamage (edict_t * targ, edict_t * inflictor)
{
  ...
  PRETRACE ();
  trace =
    gi.trace (inflictor->s.origin, vec3_origin, vec3_origin, targ->s.origin,
	      inflictor, MASK_SOLID);
  POSTTRACE ();
  if (trace.fraction == 1.0)
    return true;
...
*/
void
Killed (edict_t * targ, edict_t * inflictor, edict_t * attacker, int damage,
	vec3_t point)
{
  if (targ->health < -999)
    targ->health = -999;
...
void
blood_spray_touch (edict_t * ent, edict_t * other, cplane_t * plane,
		   csurface_t * surf)
{
  if (other == ent->owner)
    return;
...
void
spray_blood (edict_t * self, vec3_t start, vec3_t dir, int damage, int mod)
{
  edict_t *blood;
  int speed;
...
void
spray_sniper_blood (edict_t * self, vec3_t start, vec3_t dir)
{
  vec3_t forward;
  int mod = MOD_SNIPER;
...
void
VerifyHeadShot (vec3_t point, vec3_t dir, float height, vec3_t newpoint)
{
  vec3_t normdir;
  vec3_t normdir2;
...
// AQ:TNG - JBravo adding FF afterrounds
				 ...
				 ((int) (dmflags->value) &
				  (DF_NO_FRIENDLY_FIRE)) && (team_round_going
							     &&
							     ff_afterround->
							     value))))
// AQ:TNG
...
*/            
		  ...
		  if ((targ->client->pers.inventory[ITEM_INDEX (item2)])
		      ...
		      if (attacker->client)
			{
			  gi.cprintf (attacker, PRINT_HIGH,
				      "%s has a Kevlar Helmet - AIM FOR THE BODY!\n",
				      targ->client->pers.netname);
			  gi.cprintf (targ, PRINT_HIGH,
				      "Kevlar Helmet absorbed a part of %s's shot\n",
				      attacker->client->pers.netname);
			}
		      gi.sound (targ, CHAN_ITEM,
				gi.soundindex ("misc/vest.wav"), 1, ATTN_NORM,
				0);
		      damage = (int) (damage / 2);
		  ...
		  else if ((targ->client->pers.inventory[ITEM_INDEX (item2)])
			   && mod == MOD_SNIPER)
		    {
		      if (attacker->client)
			{
		      ...
		      gi.sound (targ, CHAN_VOICE,
				gi.soundindex ("misc/headshot.wav"), 1,
				ATTN_NORM, 0);
		      damage_type = LOC_HDAM;
		    }
		  else
		  ...
		  if (attacker->client)
		    {
		      attacker->client->resp.hs_streak = 0;
		      gi.cprintf (attacker, PRINT_HIGH,
				  "You hit %s in the stomach\n",
				  targ->client->pers.netname);
		    }
		  damage_type = LOC_SDAM;
		  //TempFile bloody gibbing
		  if ((mod == MOD_SNIPER) && sv_gib->value)
		    ThrowGib (targ, "models/objects/gibs/sm_meat/tris.md2",
			      damage, GIB_ORGANIC);
		  //AQ2:TNG Slicer Last Damage Location
	      ...
	      else		//(z_rel < CHEST_DAMAGE)
		{
		  ...
		  if ((targ->client->pers.inventory[ITEM_INDEX (item)])
			...
			{
			  gi.cprintf (attacker, PRINT_HIGH,
				      "%s has a Kevlar Vest - AIM FOR THE HEAD!\n",
				      targ->client->pers.netname);
			  gi.cprintf (targ, PRINT_HIGH,
				      "Kevlar Vest absorbed most of %s's shot\n",
				      attacker->client->pers.netname);
			  /*
			     if (IsFemale(targ))
			...
			{
			  gi.cprintf (attacker, PRINT_HIGH,
				      "%s has a Kevlar Vest, too bad you have AP rounds...\n",
				      targ->client->pers.netname);
			  gi.cprintf (targ, PRINT_HIGH,
				      "Kevlar Vest absorbed some of %s's AP sniper round\n",
				      attacker->client->pers.netname);
			}
		      damage = damage * .325;
		      damage_type = LOC_CDAM;
  ...
  if ((targ != attacker)
      &&
      ((deathmatch->
	value && ((int) (dmflags->value) & (DF_MODELTEAMS | DF_SKINTEAMS)))
       || coop->value))
    {
      if (OnSameTeam (targ, attacker))
	{
